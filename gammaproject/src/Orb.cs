...
namespace Gamma {
    public partial class Main : Node {
        ...
        public void OrbUpdate() {
            ...
            orb.velocity += Vector3.Down * GRAVITY_STRENGTH * globalPhysicsDeltaFloat;
            orb.velocity *= 1f - 0.25f * globalPhysicsDeltaFloat;
            if (orb.velocity.Dot(Vector3.Down) > 0f) {
                orb.velocity += Vector3.Down * (GRAVITY_STRENGTH/2) * globalPhysicsDeltaFloat;
            }
            float lift = 0.8f;
            ...
            if (inputDirection != Vector2.Zero) {
                ...
                // float rollSpeed = 4f;
                // orb.node.RotateObjectLocal(Vector3.Forward, angle * rollSpeed * globalPhysicsDeltaFloat);
            }
            if (currentSpeed < 6f) {
            ...
            Vector3 newPosition = orb.node.GlobalPosition + orb.velocity * globalPhysicsDeltaFloat;
